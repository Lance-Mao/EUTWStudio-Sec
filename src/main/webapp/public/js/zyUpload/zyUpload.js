        ...
        return this.each(function () {
            ...
            this.createHtml = function () {
                ...
                if (para.dragDrop) {
                    ...
                    html += '						<input id="no" type="text" style="display: none;" name="no" value="' + window.localStorage.getItem("studentNoByUpdate") + '">';
                    html += '					</div>';
                    html += '				</div>';
                    ...
                    html += '							<img class="upload_image" src="/public/images/zyUpload/add_img.png" style="width:expression(this.width > ' + imgWidth + ' ? ' + imgWidth + 'px : this.width)" />';
                    html += '						</div>';
                    html += '					</a>';
                    html += '				</div>';
                    html += '			</div>';
            ...
            this.funFilterEligibleFile = function (files) {
                ...
                for (var i = 0, file; file = files[i]; i++) {
                    if (file.size >= 51200000) {
                        ...
                        if (fileExt == ".png" || fileExt == ".gif" || fileExt == ".jpg" || fileExt == ".jpeg" || fileExt == ".txt" || fileExt == ".zip")
            ...
            this.funDisposePreviewHtml = function (file, e) {
                ...
                // 处理不同类型文件代表的图标
                var fileImgSrc = "public/images/zyUpload/fileType/";
                if (file.type.indexOf("rar") > 0) {
                    fileImgSrc = baseUrl + fileImgSrc + "rar.png";
                } else if (file.type.indexOf("zip") > 0) {
                    fileImgSrc = baseUrl + fileImgSrc + "zip.png";
                } else if (file.type.indexOf("text") > 0) {
                    fileImgSrc = baseUrl + fileImgSrc + "txt.png";
                } else {
                    fileImgSrc = baseUrl + fileImgSrc + "file.png";
                }
                ...
                var params = {
                    ...
                    onSuccess: function (file, response) {
                        ...
                        if (JSON.parse(response).result) {
                            $("#uploadInf").append("<p>附件上传成功</p>");
                        } else {
                            $("#uploadInf").append("<p>附件上传失败</p>");
                        }
                        // 根据配置参数确定隐不隐藏上传成功的文件
                        if (para.finishDel) {
                            // 移除效果
                        ...
                        if (JSON.parse(response).result) {
                            $.post(baseUrl + "/communication/uploadActionAjax",
                                {
                                    zyFile: JSON.parse(response).data,
                                    no: window.localStorage.getItem("studentNoByUpdate")
                                }, function (data) {
                                    if (data.result) {
